...
def get_R_w2c(yaw, pitch, theta):
    ...
    R_w2c = mtimes(mtimes(mtimes(mtimes(R_t2c , R_p2t) , R_m2p ), R_b2m) ,R_w2b)
    # R_w2c = mtimes(mtimes(mtimes(R_t2c , R_p2t) , R_m2p ), R_b2m) 
    return R_w2c
def Distance(state):
    dx = state[0] * sin(state[1])
    dy = state[0] * cos(state[1])
...
def gimbal_constrian(state, gimbal):
    ...
    temp = vertcat(X,Y,Z)
    value = mtimes(get_R_w2c(dx, dy, theta),  temp)

    return value
def gimbal_constrian1(state, gimbal):
    dx = state[0] * sin(state[1])
    dy = state[0] * cos(state[1])
    dz = state[2] 
    X = dx/dz
...
def information_matrix(robot,gimbal):
    ...
    s = np.zeros([3, 3])
    for i in range(num):
        s += temp_s[i] * (trace(temp_s[i])/alpha) 
...
for i in range(num):
    # uvd = gimbal_constrian(x[:,i],gimbal[:,i])
    # opti.subject_to(uvd[0] == 0.0)
    # opti.subject_to(uvd[1] == 0.0)
    # opti.subject_to(uvd[2] >= 2.0)
    # opti.subject_to(uvd[2] < 10.0)
    opti.subject_to(gimbal_constrian1(x[:,i],gimbal[:,i]) == 0.0)
    ...
    opti.subject_to(gimbal[0, i] > -pi) 
opti.subject_to(fabs(x[2,1] - x[2,0]) < 2)
opti.subject_to(fabs(x[2,1] - x[2,2]) < 2)
...
for i in range(100): 
    ...
    try:
        ...
        print("det", information_matrix(sol.value(x),sol.value(gimbal)))
        debug = True
    except RuntimeError as e:
        print("Solver failed with message:", str(e))
