...
bool maha_ready;
bool init, pose_init, scale_init;
...
void vo_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    if (scale_init) {
        scale = (self.z_alt(0) - 0)/ (msg->pose.position.z - 0+ 1e-6);
...
double rmse_vo(){
    ...
    rmse_sum = gt.segment(0, 2) - self.z_vo.segment(0, 2);
...
int main(int argc, char **argv){
    ...
    ros::Subscriber vo_sub = nh.subscribe<geometry_msgs::PoseStamped>("/orb_slam3/pose", 10, &vo_cb);
    ros::Subscriber imu_sub = nh.subscribe<sensor_msgs::Imu>("mavros/imu/data_raw", 10, &imu_cb);
